...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, kv=None, key_padding_mask=None, return_attention=False):
        ...
        # [batch, (source or target) seq, input_size] -> [batch, seq, hidden]
        if kv is not None:
            k, q, v = self.k(kv), self.q(q), self.v(kv)
        else:
            k, q, v = self.k(q), self.q(q), self.v(q)
        ...
        if key_padding_mask is not None:
            ...
            if key_padding_mask.shape[0] == bs:
                mask = key_padding_mask.repeat(self.num_heads, 1).unsqueeze(2)
            else:
                mask = key_padding_mask.unsqueeze(2)
