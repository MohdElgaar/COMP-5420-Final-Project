...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, decoder_hidden_states, encoder_hidden_states, key_padding_mask=None):
        ...
        z = decoder_hidden_states
        x = self.self_attention(decoder_hidden_states)
        x = self.self_att_layer_norm(x + z)
        ...
        x = self.cross_att_layer_norm(x + z)

        z = x
        x = self.fcn(x)
        x = self.dropout(x)
        x = self.fcn_layer_norm(x + z)
...
class TransfomerEncoderDecoderModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.encoder_layers = nn.ModuleList(
                [TransformerEncoderLayer(hidden, num_heads, fcn_hidden, dropout)
                    for _ in range(num_layers)]
                )
        self.decoder_layers = nn.ModuleList(
                [TransformerDecoderLayer(hidden, num_heads, fcn_hidden, dropout)
                    for _ in range(num_layers)]
                )
        # YOUR CODE ENDS HERE
    ...
    def _encode(self, input_ids, key_padding_mask):
        ...
        for m in self.encoder_layers:
            x = m(x, key_padding_mask=key_padding_mask)
